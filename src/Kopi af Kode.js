/****************************************************
 * TMFG PERSONAL MACHINE — HUMANIZED V2 + RELATIVE INTERNAL LINKS
 * - Auto plan: Title + Type + CategorySlug + Angle
 * - Anti-repeat + real diversity (angles)
 * - Stronger "lived experience" layer to reduce AI-feel
 * - Creates post on WP.com (category via slug)
 * - Optional featured image (family caricature style)
 * - Internal linking: related block + limited inline links
 * - Internal links are converted to RELATIVE URLs to reduce self-pingbacks
 * - Hubs: auto-create + auto-update per category
 *
 * Sheet headers (Row 1):
 * A Title | B Status | C Post URL | D Notes | E Type | F CategorySlug | G Angle
 *
 * How to use:
 * - Put "Queued" in column B on an empty row
...
// ===== Editorial angles (forces real diversity) =====
var ANGLES = [
  ...
  try {
    ...
    for (var i = 1; i < rows.length; i++) {
      ...
      try {
        ...
        sheet.getRange(rowIndex, 4).setValue(
          "OK | " + plan.type + " | " + plan.categorySlug + " | " + plan.angle + " | " + plan.toneMode + " | Img:" + (mediaId ? "yes" : "no")
        );
...
function generateEditorialPlan_(history) {
  ...
  var recentTitles = history.titles.slice(0, 20);
  ...
  var prompt =
    ...
    '  "friction": "One concrete thing that was difficult, annoying, or not working",\n' +
    '  "smallWin": "One tiny thing that helped or changed the situation",\n' +
    '  "sceneDetail": "One specific real-world detail from home/family life",\n' +
    '  "toneMode": "ONE of: ' + TONE_MODES.join(", ") + '"\n' +
    "}\n\n" +
    "Hard rules:\n" +
    "- Avoid mentioning any family member names.\n" +
    "- Do NOT reuse wording patterns from recent titles.\n" +
    "- Do NOT use the pattern: 'Our ... Sunday ... Week' or 'Simple ... Calm Week' or 'Quiet ... Week'.\n" +
    "- Prefer specificity: constraints, time limits, friction points, mistakes, a moment.\n" +
    "- Title must be a different IDEA, not a synonym rewrite.\n" +
    "- The idea must be grounded in a lived family moment, not a generic advice article.\n" +
    "- Include one concrete friction point and one imperfect real-life detail.\n" +
    "- Avoid polished content-marketing style titles.\n\n" +
    "Banned (do not pick):\n" +
  ...
  for (var attempt = 1; attempt <= 3; attempt++) {
    ...
    var jsonString = extractJsonObject_(raw);
    var plan = JSON.parse(jsonString);
...
function generatePersonalPostJson_(plan, history) {
  ...
  var schema =
    ...
    "Rules:\n" +
    "- No URLs.\n" +
    "- Avoid naming any family members.\n" +
    "- Short paragraphs.\n" +
    "- Voice: " + BLOG_VOICE + "\n" +
    "- Must strongly match the ANGLE, HOOK, SCENARIO, FRICTION, SMALL WIN and SCENE DETAIL.\n" +
    "- Do NOT rehash the same idea as recent titles.\n" +
    "- Write like this happened in a real family home.\n" +
    "- Use 'we' and 'our' naturally.\n" +
    "- Include one imperfect or messy detail.\n" +
    "- Include one concrete moment, not generic advice.\n" +
    "- Do NOT sound polished, generic, or overly bloggy.\n" +
    "- Avoid generic filler like 'family life can be busy' or 'it is important to'.\n" +
    "- Prefer specific details: hallway shoes, a messy counter, cold pasta, a too-late bedtime, a rushed school morning.\n" +
    "- Include one small failure, one emotional truth, and one practical adjustment.\n\n" +
    "Recent titles (avoid overlap):\n" + recentTitles + "\n";

  var prompt =
    "Blog: The Modern Family's Guide\n" +
    "Title: \"" + plan.title + "\"\n" +
    "Type: " + plan.type + "\n" +
  ...
  var raw = geminiText_(prompt);
  var jsonString = extractJsonObject_(raw);
  var ai = JSON.parse(jsonString);

  if (!ai || typeof ai !== "object") throw new Error("Invalid AI JSON.");
  ...
  ai.sections = ai.sections.slice(0, 4).map(function(s){
    return { h2: String((s && s.h2) ? s.h2 : "").trim(), p: String((s && s.p) ? s.p : "").trim() };
  }).filter(function(s){ return s.h2 && s.p; });

  ai.faq = ai.faq.slice(0, 3).map(function(f){
    return { q: String((f && f.q) ? f.q : "").trim(), a: String((f && f.a) ? f.a : "").trim() };
...
function angleDirective_(angle) {
  switch (angle) {
    case "SYSTEMS_AND_ROUTINES": return "Focus on repeatable systems. Include a simple framework and a tiny habit to lock it in.";
    case "LOW_ENERGY_MODE": return "Write for exhausted parents. Offer low-effort steps, defaults, and permission to keep it small.";
    case "TIME_SAVING": return "Use time boxes (5/10/20 minutes) and show where the time comes back during the week.";
    case "MONEY_SAVING": return "Practical savings: 3 concrete swaps + one trap to avoid. No deprivation vibe.";
    case "CONFLICT_REDUCTION": return "Reduce friction: one rule, one exact sentence (script), one boundary that prevents arguments.";
    case "KID_BEHAVIOR": return "Behavior via environment: setup, cues, predictable rhythm, and what to do when it fails.";
    case "HOME_RESET": return "Home reset: a tiny checklist and a trigger (when to do it). Keep it realistic.";
    case "MEAL_SIMPLIFICATION": return "Meals as a system: default rotation, one shortcut, less decision fatigue.";
    case "TRAVEL_PREP": return "Outings prep: packing logic, pre-leave routine, 'what we forgot last time' fix.";
    case "EMOTIONAL_REFLECTION": return "A real feeling + what changed. Ground it and end with practical takeaways.";
    case "MINIMALISM": return "Less but better: remove one thing, simplify one shelf/system, highlight relief and function.";
    case "REAL_WORLD_EXAMPLE":
    default: return "Tell one concrete real-world moment, then extract a simple rule and repeatable practice.";
  }
}

function postTypeDirective_(type) {
  switch (type) {
    case "SUNDAY_RESET": return "Include a realistic 20–45 minute reset, a checklist section, and a 'do one thing' tip.";
    case "REAL_LIFE_HACKS": return "Write 5–7 hacks with punchy subheadings. Each hack: 2–4 sentences + quick example.";
    case "FAMILY_ACTIVITY": return "Include setup, materials, steps, ages, cleanup tips, and 'why it works'.";
    case "RELATIONSHIP_MINI": return "Short reflection + 3 actionable micro-habits. No therapy jargon. Practical and respectful.";
    case "RECIPE_NOTE": return "Include ingredients + steps + one shortcut tip + a note about picky kids/leftovers.";
    case "PERSONAL_STORY":
    default: return "Hook → messy middle → what we learned → 3–5 practical tips. Authentic.";
  }
}

function toneDirective_(toneMode) {
  switch (toneMode) {
    case "TIRED": return "Tone: low-energy, slightly worn out, honest, gentle, no drama.";
    case "LIGHTLY_FUNNY": return "Tone: dry, warm, lightly funny, but still practical.";
    case "REFLECTIVE": return "Tone: thoughtful and grounded, but still concrete and useful.";
    case "RELIEVED": return "Tone: calm relief after finding something that helped. No big claims.";
    case "PRACTICAL":
    default: return "Tone: practical, direct, and quietly reassuring.";
  }
}
...
function generateHeroImage_(title, keyword, categorySlug, angle) {
  ...
  var prompt =
    ...
    "Style: " + FAMILY_IMAGE_LOCK_STYLE + ". Friendly, slightly exaggerated features. Modern Scandinavian home vibe.\n" +
    familyLine +
...
function getInternalLinkCandidates_(rows, plan, maxLinks) {
  ...
  function tryPick(filterFn) {
    ...
    if (best) { used[best.url] = true; picked.push(best); }
  }
  ...
  while (picked.length < maxLinks) {
    ...
    for (var k = 0; k < pool.length; k++) {
      ...
      var sc2 = score(it2);
      if (sc2 > bestScore2) { bestScore2 = sc2; best2 = it2; }
    }
    if (!best2) break;
    used[best2.url] = true;
    picked.push(best2);
...
function normalizeTitleTokensLite_(s) {
  var stop = { "a":1,"an":1,"and":1,"the":1,"to":1,"of":1,"for":1,"in":1,"on":1,"with":1,
...
function getWpSiteHomeCached_() {
  ...
  if (res.getResponseCode() !== 200) {
    throw new Error("Could not fetch WP site info for relative URL conversion.");
  }
...
function buildHubHtml_(categorySlug, mainSheet) {
  ...
  for (var k = 0; k < angleKeys.length; k++) {
    ...
    sections +=
      "<h2 style=\"margin-top:26px;\">" + escapeHtml_(nice) + "</h2>" +
...
function buildPersonalHtml_(plan, ai, hub, candidates) {
  ...
  var sections = (ai.sections || []).map(function(s, idx){
    ...
    return (
      '<h2 style="margin:22px 0 8px 0;">' + h2 + "</h2>" +
  ...
  var faqItems = (ai.faq || []).slice(0, 3).map(function(f){
    return (
      '<h3 style="margin:16px 0 6px 0;font-size:16px;">' + escapeHtml_(f.q || "") + "</h3>" +
...
function uploadWpMedia_(blob, title) {
  ...
  var code = res.getResponseCode();
  var body = res.getContentText();
  if (code !== 200 && code !== 201) throw new Error("Media upload failed HTTP " + code + ": " + truncate_(body, 900));
...
function createWpPost_(title, html, featuredMediaId, status, categorySlug) {
  ...
  var code = res.getResponseCode();
  var body = res.getContentText();
  if (code !== 200 && code !== 201) throw new Error("Create post failed HTTP " + code + ": " + truncate_(body, 1000));
