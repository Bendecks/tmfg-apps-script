/****************************************************
 * TMFG V2 — ENGINE
 * AI generation, images, HTML, WordPress helpers, utils
 ****************************************************/
...
function getWpSiteHomeCached_() {
  ...
  if (res.getResponseCode() !== 200) throw new Error("Could not fetch WP site info.");

  var json = JSON.parse(res.getContentText());
  var siteUrl = String(json.URL || json.url || "").trim();
  var hostMatch = siteUrl.match(/^https?:\/\/([^\/]+)$/i) || siteUrl.match(/^https?:\/\/([^\/]+)\//i);

  var info = { url: siteUrl, host: hostMatch ? String(hostMatch[1] || "").toLowerCase() : "" };
